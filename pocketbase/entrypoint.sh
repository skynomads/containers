...
if [ ! -f /var/pocketbase/data.db ]; then
  ...
  curl -v -X PATCH http://localhost:8080/api/settings \
    ...
    -d "{\"smtp\":{\"enabled\":true,\"host\":\"$POCKETBASE_SMTP_HOST\",\"port\":$POCKETBASE_SMTP_PORT,\"username\":\"$POCKETBASE_SMTP_USERNAME\",\"password\":\"$POCKETBASE_SMTP_PASSWORD\"},\"s3\":{\"bucket\":\"$POCKETBASE_S3_BUCKET\",\"endpoint\":\"$POCKETBASE_S3_ENDPOINT\",\"accessKey\":\"$POCKETBASE_S3_ACCESS_KEY\",\"secret\":\"$POCKETBASE_S3_SECRET\"}}"
